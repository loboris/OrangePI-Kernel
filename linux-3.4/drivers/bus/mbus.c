...
#define SUNXI_MBUS_PBASE            SUNXI_DRAMCOM_PBASE
#define SUNXI_MBUS_VBASE            IO_ADDRESS(SUNXI_MBUS_PBASE)
#define MBUS_MAST_CFG0_REG(n)       IO_ADDRESS((SUNXI_MBUS_PBASE + 0x0010 + (0x8 * n))) /* Config n = 0 ~ 15(w5)/13(w6)     */
#define MBUS_MAST_CFG1_REG(n)       IO_ADDRESS((SUNXI_MBUS_PBASE + 0x0014 + (0x8 * n))) /* BWL Config n = 0 ~ 15(w5)/13(w6) */
#define MBUS_BW_CFG_REG             IO_ADDRESS((SUNXI_MBUS_PBASE + 0x0090)) /* Bandwith Window base on MCLK cycles */
#define MBUS_MAST_ACEN_CFG_REG      IO_ADDRESS((SUNXI_MBUS_PBASE + 0x0094)) /* Master Access Enable, 0:dis, 1:en   */
#if defined CONFIG_ARCH_SUN8IW5P1
#define MBUS_MAST_ACPR_CFG_REG      IO_ADDRESS((SUNXI_MBUS_PBASE + 0x0098)) /* Master Access Priority, 0:low, 1:hg */
#endif
#define MBUS_PMU_CNTEB_CFG_REG      IO_ADDRESS((SUNXI_MBUS_PBASE + 0x009c)) /* Counter Enable, 0x0001:enable all   */
#define MBUS_PMU_CNT_REG(n)         IO_ADDRESS((SUNXI_MBUS_PBASE + 0x00a0 + (0x4 * n))) /* Counter n = 0 ~ 6       */
/* generaly, the below registers are not needed to modify */
#define MBUS_SW_CLK_ON_REG          IO_ADDRESS((SUNXI_MBUS_PBASE + 0x00c0)) /* Software Clock ON, 0:open by hws    */
#define MBUS_SW_CLK_OFF_REG         IO_ADDRESS((SUNXI_MBUS_PBASE + 0x00c4)) /* Sofrware Clock OFF, 1:dis-access    */
#define MBUS_RESOURCE_SIZE          (MBUS_SW_CLK_OFF_REG - SUNXI_MBUS_VBASE)
...
struct sunxi_mbus_port {
	...
	unsigned long phys;     /* SUNXI_MBUS_PBASE */
...
#define mbus_pmu_getstate() (readl_relaxed(MBUS_PMU_CNTEB_CFG_REG) & 1)
#define mbus_pmu_enable() \
	writel_relaxed(((readl_relaxed(MBUS_PMU_CNTEB_CFG_REG)) | 1), MBUS_PMU_CNTEB_CFG_REG)
...
int notrace mbus_port_setbwlen(mbus_port_e port, bool en)
{
	...
	writel_relaxed(value | (en << MBUS_BWLEN_SHIFT), MBUS_MAST_CFG0_REG(port));
...
int notrace mbus_port_setpri(mbus_port_e port, bool pri)
{
	...
	mutex_lock(&mbus_seting);
#if defined CONFIG_ARCH_SUN8IW5P1
	value = readl_relaxed(MBUS_MAST_ACPR_CFG_REG);
	value &= ~(1 << port);
	writel_relaxed(value | (pri << port), MBUS_MAST_ACPR_CFG_REG);
...
int notrace mbus_port_setqos(mbus_port_e port, unsigned int qos)
{
	...
	writel_relaxed(value | (qos << MBUS_QOS_SHIFT), MBUS_MAST_CFG0_REG(port));
	mutex_unlock(&mbus_seting);
...
int notrace mbus_port_setwt(mbus_port_e port, unsigned int wt)
{
	...
	writel_relaxed(value | (wt << MBUS_WT_SHIFT), MBUS_MAST_CFG0_REG(port));
	mutex_unlock(&mbus_seting);
...
int notrace mbus_port_setacs(mbus_port_e port, unsigned int acs)
{
	...
	writel_relaxed(value | (acs << MBUS_ACS_SHIFT), MBUS_MAST_CFG0_REG(port));
	mutex_unlock(&mbus_seting);
...
int notrace mbus_port_setbwl0(mbus_port_e port, unsigned int bwl0)
{
	...
	writel_relaxed(value | (bwl0 << MBUS_BWL0_SHIFT), MBUS_MAST_CFG0_REG(port));
	mutex_unlock(&mbus_seting);
...
int notrace mbus_port_setbwl1(mbus_port_e port, unsigned int bwl1)
{
	...
	value = readl_relaxed(MBUS_MAST_CFG1_REG(port));
	value &= ~(MBUS_BWL_MAX << MBUS_BWL1_SHIFT);
	writel_relaxed(value | (bwl1 << MBUS_BWL1_SHIFT), MBUS_MAST_CFG1_REG(port));
	mutex_unlock(&mbus_seting);
...
int notrace mbus_port_setbwl2(mbus_port_e port, unsigned int bwl2)
{
	...
	writel_relaxed(value | (bwl2 << MBUS_BWL2_SHIFT), MBUS_MAST_CFG1_REG(port));
	mutex_unlock(&mbus_seting);
                             ...
                             unsigned int bwl1, unsigned int bwl2)
{
	...
	if ((bwl0 > MBUS_BWL_MAX) || (bwl1 > MBUS_BWL_MAX) || (bwl2 > MBUS_BWL_MAX))
		return -EPERM;
...
int notrace mbus_set_bwlwen(bool enable)
{
	...
	mutex_lock(&mbus_seting);
	value = readl_relaxed(MBUS_BW_CFG_REG);

	writel_relaxed(enable ? (value | (1 << MBUS_BWEN_SHIFT)) :\
		 (value & ~(1 << MBUS_BWEN_SHIFT)), MBUS_BW_CFG_REG);
	mutex_unlock(&mbus_seting);
...
int notrace mbus_set_bwlwsiz(unsigned int size)
{
	...
	writel_relaxed(value | size, MBUS_BW_CFG_REG);
	mutex_unlock(&mbus_seting);
...
static void notrace mbus_port_control(mbus_port_e port, bool enable)
{
	 ...
	 */
	mutex_lock(&mbus_seting);
	value = readl_relaxed(MBUS_MAST_ACEN_CFG_REG);
	if (enable) {
		value |= (1 << port);
	} else {
		value &= ~(1 << port);
	}
	writel_relaxed(value, MBUS_MAST_ACEN_CFG_REG);
	mutex_unlock(&mbus_seting);
}
...
static unsigned int mbus_update_device(struct mbus_data *data, mbus_pmu_e port)
{
	...
	/* read pmu conter */
	value = readl_relaxed(MBUS_PMU_CNT_REG(port));
                                   ...
                                   unsigned int index, char *buf)
{
	...
	case MBUS_PORT_PRI:
		...
		printk("func:%s, line:%d this platform is not supported!!\n", \
		        __func__, __LINE__);
		{
#endif
...
static SENSOR_DEVICE_ATTR(pmu_cpuddr, S_IRUGO, mbus_show_value, NULL,
			  MBUS_PMU_CPU);

/* GPU bandwidth of DDR channel 0 */
static SENSOR_DEVICE_ATTR(pmu_gpuddr, S_IRUGO, mbus_show_value, NULL,
			  MBUS_PMU_GPU);

/* DE bandwidth of DDR channel 0 */
static SENSOR_DEVICE_ATTR(pmu_ve_ddr, S_IRUGO, mbus_show_value, NULL,
			  MBUS_PMU_VE);

/* VE & CSI & FD bandwidth of DDR channel 0 */
static SENSOR_DEVICE_ATTR(pmu_de_ddr, S_IRUGO, mbus_show_value, NULL,
			  MBUS_PMU_DISP);

/* other master bandwidth of DDR channel 0 */
static SENSOR_DEVICE_ATTR(pmu_othddr, S_IRUGO, mbus_show_value, NULL,
			  MBUS_PMU_OTH);

/* CPU bandwidth of DDR channel 1 */
static SENSOR_DEVICE_ATTR(pmu_totddr, S_IRUGO, mbus_show_value, NULL,
			  MBUS_PMU_TOTAL);

/* get all masters' priority or set a master's priority */
static SENSOR_DEVICE_ATTR(port_prio, S_IRUGO | S_IWUSR, mbus_show_value, mbus_store_value,
			  MBUS_PORT_PRI);

/* get all masterss' qos or set a master's qos */
static SENSOR_DEVICE_ATTR(port_qos, S_IRUGO | S_IWUSR, mbus_show_value, mbus_store_value,
			  MBUS_PORT_QOS);

/* get all masterss' threshold or set a master's threshold */
static SENSOR_DEVICE_ATTR(port_watt, S_IRUGO | S_IWUSR, mbus_show_value, mbus_store_value,
			  MBUS_PORT_WT);

/* get all masterss' threshold or set a master's threshold */
static SENSOR_DEVICE_ATTR(port_acs, S_IRUGO | S_IWUSR, mbus_show_value, mbus_store_value,
			  MBUS_PORT_ACS);

/* get all masters' requeset number or set a master's number */
static SENSOR_DEVICE_ATTR(port_bwl0, S_IRUGO | S_IWUSR, mbus_show_value, mbus_store_value,
			  MBUS_PORT_BWL0);

/* get all masters' requeset number or set a master's number */
static SENSOR_DEVICE_ATTR(port_bwl1, S_IRUGO | S_IWUSR, mbus_show_value, mbus_store_value,
			  MBUS_PORT_BWL1);

/* get all masters' requeset number or set a master's number */
static SENSOR_DEVICE_ATTR(port_bwl2, S_IRUGO | S_IWUSR, mbus_show_value, mbus_store_value,
			  MBUS_PORT_BWL2);
...
static int mbus_pmu_probe(struct platform_device *pdev)
{
	...
	hw_mbus_pmu.hwmon_dev = hwmon_device_register_attr(&pdev->dev, "mbus_pmu");
...
/* MBUS MEM resouces */
static struct resource mbus_resource[] = {
	[0] = {
		.start = SUNXI_MBUS_PBASE,
		.end   = SUNXI_MBUS_PBASE + MBUS_RESOURCE_SIZE,
		.flags = IORESOURCE_MEM,
	},
};

static struct platform_device mbus_pmu_device = {
	.name           = DRIVER_NAME_PMU,
	.id             = PLATFORM_DEVID_NONE,
	.num_resources  = ARRAY_SIZE(mbus_resource),
	.resource       = mbus_resource,
};
...
static int __init mbus_platform_init(void)
{
	int ret;

	ret = platform_device_register(&mbus_pmu_device);
	if (IS_ERR_VALUE(ret)) {
...
device_initcall(mbus_platform_init);
