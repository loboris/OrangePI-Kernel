	...
	for (j=0,k=0;j<chn;j++)
	{
		...
		for(i=0;i<layno;i++)
		{
			if(LAYER_ATTR_DIRTY & data[k].flag)
			{
				...
				de_rtmx_set_lay_laddr(screen_id,j,i,lay_cfg[k].fmt,crop[j][i], lay_cfg[k].pitch,data[k].config.info.fb.align,(de_3d_in_mode)data[k].config.info.fb.flags,lay_cfg[k].laddr_t,haddr[i]);
			}
