...
extern int NAND_Print(const char * str, ...);
#define nand_debug(...)							NAND_Print(__VA_ARGS__)	
...
{
	printf("call null NAND_build_all_partition\n");
	return 0;	
}
...
int NAND_UbootInit(int boot_mode)
{
	...
	nand_debug("NAND_UbootInit end: 0x%x\n", ret);
...
__s32  burn_boot0_lsb_mode(__u32 read_retry_type, __u32 Boot0_buf, __u32 length)
{
    __u32 i,k,page_cnt_per_copy,read_retry_mode,pages_per_block;
    __u8  oob_buf[32];
    __u32 page_size;
    struct boot_physical_param  para;
	void * buf;
    ...
    nand_debug("burn boot0 lsb mode,retry mode:%x!\n",read_retry_mode);
    ...
    for( i = NAND_BOOT0_BLK_START;  i < (NAND_BOOT0_BLK_START + NAND_BOOT0_BLK_CNT);  i++ )
    {
        ...
        /* 在块中烧写boot0备份, lsb mode下，每个块只能写连续的page */
		...
		{
			for(k=page_cnt_per_copy; k< pages_per_block;k++)
			{
				para.chip  = 0;
				para.block = i;
				if(_Is_lsb_page_or_not(k))
				{
					...
					{
						nand_debug("Warning. Fail in writing page dummy data %d in block %d.\n", k, i );
					}
				...
				else
				{
					...
					if(PHY_SimpleWrite_0xFF(&para_1) < 0)
						nand_debug("Warning. Fail in writing 0xFF to page %d in block %d.\n", k, i );
...
exit:
	...
	PHY_WaitAllRbReady();	
	nand_debug("lsb disalbe 0\n");
	NFC_LSBExit(read_retry_type);
	PHY_WaitAllRbReady();	
	nand_debug("lsb disalbe 1\n");
	NFC_LSBExit(read_retry_type);
	PHY_WaitAllRbReady();	
	nand_debug("lsb disalbe 2\n");
	NFC_LSBExit(read_retry_type);
	PHY_WaitAllRbReady();	
...
int NAND_BurnBoot0(uint length, void *buffer)
{
	...
	if( (read_retry_type>0)&&(read_retry_mode < 0x10))
	{
		...
		else if( burn_boot0_lsb_mode(read_retry_type, (__u32)buffer ,length ))
	        goto error;
	}
	else
	{
...
__s32 burn_uboot_in_one_blk_normal_mode(__u32 UBOOT_buf, __u32 length)
{
    ...
    for( i = NAND_UBOOT_BLK_START;  i < (NAND_UBOOT_BLK_START + NAND_UBOOT_BLK_CNT);  i++ )
    {
        ...
        nand_debug("fill uboot(normal mode) block with dummy data\n");
		for( k = pages_per_copy;  k < pages_per_block;  k++ )
		{
			...
			if( PHY_SimpleWrite( &para ) <0 )
			{
				nand_debug("Warning. Fail in writing page %d in block %d.\n", k, i );
   			}
   		}		
    ...
    for( i = NAND_UBOOT_BLK_START;  i < (NAND_UBOOT_BLK_START + NAND_UBOOT_BLK_CNT);  i++ )
    {
      	...
      	for( k = 0;  k < pages_per_copy;  k++ )
		{
			...
			{
				nand_debug("Warning. Fail in read page %d in block %d.\n", k, i );
   			}
   		}
    }
	free(buf);
...
{
    __u32 i, k,read_retry_type;
    __u8  oob_buf[32];
    __u32 page_size, pages_per_block, pages_per_copy;
	void * buf;
    struct boot_physical_param  para;

	nand_debug("burn uboot lsb mode!\n");

	read_retry_type = NAND_GetReadRetryType();

	nand_debug("lsb enalbe \n");
	nand_debug("read retry mode: 0x%x\n", read_retry_type);
	if( NFC_LSBInit(read_retry_type) )
	{
		nand_debug("lsb init failed.\n");
		goto error;
	}
	NFC_LSBEnable(0, read_retry_type);
    ...
    if((oob_buf[0]!=0xff)||(oob_buf[1]!= 0x00))
	{
		nand_debug("get flash driver version error!");
		goto error;
	}
	...
	page_size = NAND_GetPageSize();
	{
		if(page_size %1024)
		{
			nand_debug("get flash page size error!");
			goto error;
		}
	}
	...
	if(pages_per_block%64)
	{
		nand_debug("get page cnt per block error %x!", pages_per_block);
		goto error;
	}

	nand_debug("pages_per_block: 0x%x\n", pages_per_block);

	/* 计算每个备份所需page */
	if(length%page_size)
	{
		nand_debug("uboot length check error!\n");
		goto error;
	}
	pages_per_copy = length/page_size;
	if(pages_per_copy>pages_per_block)
	{
		nand_debug("pages_per_copy check error!\n");
		goto error;
	}

	nand_debug("pages_per_copy: 0x%x\n", pages_per_copy);
    ...
    for( i = NAND_UBOOT_BLK_START;  i < (NAND_UBOOT_BLK_START + NAND_UBOOT_BLK_CNT);  i++ )
    {
		...
		if( PHY_SimpleErase( &para ) <0 )
		{
		    nand_debug("Fail in erasing block %d.\n", i );
    		continue;
    	}
		...
		for( k = 0;  k < pages_per_copy;  k++ )
		{
			...
			//nand_debug("get lsb page ,src page num:%x ,phy page num:%x \n",k,para.page);
			if(para.page==0xffffffff){
				nand_debug("get lsb page error,src page num:%x \n",k);
				break;
			}
			para.mainbuf = (void *) (UBOOT_buf + k * page_size);
			para.oobbuf = oob_buf;
		...
		nand_debug("fill uboot(lsb mode) block with dummy data\n");
		for(k = pages_per_copy; k < pages_per_block ; k++)
		{
			para.chip  = 0;
			para.block = i;
			para.page  = _lsb_page_trans(k);
			if(para.page==0xffffffff){
				nand_debug("not lsb page ,the last lsb page num:%x \n",_lsb_page_trans(k-1));
				break;
			}
...
__s32 burn_uboot_in_one_blk_lsb_mode_F16(__u32 UBOOT_buf, __u32 length)
{
	...
	nand_debug("burn uboot lsb mode F16!\n");
...
//	int blk_index, page_index;
	...
	{
		if((read_retry_type)&&((read_retry_mode==0x0)||(read_retry_mode==0x1)||(read_retry_mode==0x2)||(read_retry_mode==0x3)))
			ret = burn_uboot_in_one_blk_normal_mode((__u32)buffer, length);			
		else if((read_retry_type)&&(read_retry_mode == 0x4))
			ret = burn_uboot_in_one_blk_normal_mode((__u32)buffer, length);
		else
			ret = burn_uboot_in_one_blk_normal_mode((__u32)buffer, length);
	}
