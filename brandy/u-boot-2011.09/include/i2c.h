 ...
 *	 bus_id£º bus id for twi bus address
 ...
 */
static inline u8 i2c_reg_read(uint bus_id, u8 addr, u8 reg)
{
	u8 buf;

#ifdef CONFIG_8xx
	/* MPC8xx needs this.  Maybe one day we can get rid of it. */
	i2c_init(bus_id, CONFIG_SYS_I2C_SPEED, CONFIG_SYS_I2C_SLAVE);
#endif
	...
	i2c_read(bus_id, addr, reg, 1, &buf, 1);

	return buf;
}

static inline void i2c_reg_write(uint bus_id, u8 addr, u8 reg, u8 val)
{
#ifdef CONFIG_8xx
	/* MPC8xx needs this.  Maybe one day we can get rid of it. */
	i2c_init(bus_id, CONFIG_SYS_I2C_SPEED, CONFIG_SYS_I2C_SLAVE);
	...
	i2c_write(bus_id, addr, reg, 1, &val, 1);
}
