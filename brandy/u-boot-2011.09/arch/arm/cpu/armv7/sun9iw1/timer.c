...
static __inline void __msdelay_nssecure(__u32 ms);
static __inline void __usdelay_nssecure(__u32 us);
static __inline void __msdelay_secure(__u32 ms);
static __inline void __usdelay_secure(__u32 us);
static __inline void __delay_null(u32 time);
...
int runtime_tick(void)
{
	if(__usdelay_pt == __usdelay_nssecure)
	{
		...
		time = 0xffffffff - readl(TMRC_CURNT(4));
