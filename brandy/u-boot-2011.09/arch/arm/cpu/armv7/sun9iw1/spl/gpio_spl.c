...
int boot_set_one_gpio(normal_gpio_set_t  *user_gpio,  __s32 set_gpio)
{
    ...
    port     = user_gpio->port;                         //读出端口数值
    port_num = user_gpio->port_num;                     //读出端口中的某一个GPIO
	...
	if(!cpus_flag)
	{
		tmp_func_addr    = PIO_REG_CFG(port, port_num_func);   //更新功能寄存器地址
		tmp_pull_addr    = PIO_REG_PULL(port, port_num_pull);  //更新pull寄存器
		tmp_dlevel_addr  = PIO_REG_DLEVEL(port, port_num_pull);//更新level寄存器
		tmp_data_addr    = PIO_REG_DATA(port);                 //更新data寄存器
	}
	else
	{
#if defined(SUNXI_R_PIO_BASE)
		tmp_func_addr    = R_PIO_REG_CFG(port, port_num_func);   //更新功能寄存器地址
		tmp_pull_addr    = R_PIO_REG_PULL(port, port_num_pull);  //更新pull寄存器
		tmp_dlevel_addr  = R_PIO_REG_DLEVEL(port, port_num_pull);//更新level寄存器
		tmp_data_addr    = R_PIO_REG_DATA(port);                 //更新data寄存器
#else
		return -1;
#endif
	}
	...
	if(tmp_func_addr)                         //只要更新过寄存器地址，就可以对硬件赋值
	{                                               //那么把所有的值全部回写到硬件寄存器
		GPIO_REG_WRITE(tmp_func_addr,   tmp_func_data);   //回写功能寄存器
		GPIO_REG_WRITE(tmp_pull_addr,   tmp_pull_data);   //回写pull寄存器
		GPIO_REG_WRITE(tmp_dlevel_addr, tmp_dlevel_data); //回写driver level寄存器
		if(data_change)
		{
			GPIO_REG_WRITE(tmp_data_addr, tmp_data_data); //回写data寄存器
		}
	}
