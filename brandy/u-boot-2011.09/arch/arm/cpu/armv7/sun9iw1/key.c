...
int sunxi_key_init(void)
{
    ...
    uint reg_val = 0;
   // open LRADC gating bit
    reg_val = readl(CCM_APB0_GATE0_CTRL);
    reg_val |=  1<<15;
    writel(reg_val,CCM_APB0_GATE0_CTRL);
    //open LRADC reset bit
    reg_val = readl(CCM_APB0_RST_REG0);
    reg_val |=  1<<15;
    writel(reg_val,CCM_APB0_RST_REG0);
